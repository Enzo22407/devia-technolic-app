...
<head>
    <meta charset="UTF-8">
    <title>Mise à Jour Dossier — Devia Technologic</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Mise à Jour de votre Dossier Requête</p>
        </div>
        <div class="content">
            <h2>Bonjour {{ $studentRequest->student->name ?? 'Étudiant' }},</h2>
            
            <p>Le statut de votre requête numéro <strong>#{{ $studentRequest->reference_code }}</strong> a été mis à jour par l'administration.</p>

            <div class="badge-box">
                <p style="margin: 0 0 8px;"><strong>Objet :</strong> {{ $studentRequest->subject }}</p>
                <p style="margin: 0 0 8px;"><strong>Nouveau Statut :</strong> <span style="color: #4f46e5; font-weight: 800;">{{ $studentRequest->status_label }}</span></p>
                @if($comment)
                    <p style="margin: 0;"><strong>Commentaire / Remarques :</strong> "{{ $comment }}"</p>
                @endif
            </div>

            <div style="text-align: center;">
                <a href="{{ route('requests.show', $studentRequest->id) }}" class="btn">Voir Mon Dossier Complet</a>
            </div>
        </div>
        <div class="footer">
